...
2. For this project's current maturity (Alpha), bias toward `MINOR` for meaningful UX/workflow additions and `PATCH` for isolated fixes.

3. Before release builds, ensure `CMakeLists.txt` version is greater than `Docs/LAST_RELEASED_VERSION.txt`.

4. In release/commit notes, always include brief rationale for why that bump level was chosen.

5. **Hard rule**: each commit must advance version monotonically from the previous commit. Use patch bumps for normal development commits unless a minor/major bump is explicitly justified.

6. Keep strict semantic version syntax in all version fields (`X.Y.Z` only).
...
2. For major edits in large files (e.g., `PluginEditor.cpp`, `PluginProcessor.cpp`), include a short extraction/refactor note in commit notes.
...
1. Do not alter chord naming/modifier behavior unless explicitly requested and covered by tests/fixtures.
2. Do not change `.chbpreset` compatibility.
3. Do not scatter standalone-only plugin-hosting `#if` logic; isolate behind services where possible.
